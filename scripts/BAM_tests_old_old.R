...
#bsub -q long -n 8 -R rusage[mem=20000] -W 8:00 /home/cb63a/batchBAM.sh 
...
for (phase in allpepsi2){
  setwd(phase)
  output_directory=paste('/home/cb63a/BAMtests/regime_switch/')
...
new_phase <- c('IowaRiver.nc', 'Jamuna.nc', 'Kanawha.nc', 'Kushiyara.nc', 'MississippiDownstream.nc', 'MississippiUpstream.nc', 'MissouriDownstream.nc', 'MissouriMidsection.nc', 'Tanana.nc', 'Wabash.nc')

for (i in 1:1:(length(new_phase))) {
  if (i == 1 | i == 30 | i == 31){
    print('already done')
    next
  }
